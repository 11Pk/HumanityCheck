...
//   if      (score >= 70) { label = "Likely Real Profile"; color = "#00ba7c"; emoji = "✅"; }
//   else if (score >= 40) { label = "Suspicious";        color = "#ffad1f"; emoji = "⚠️"; }
//   else                  { label = "Likely Fake Profile"; color = "#e0245e"; emoji = "🤖"; }
...
// ─────────────────────────────────────────────────────────────
//  HumanityCheck — content.js
//  Fix: replaced all setTimeout guessing with waitForElement()
//  so scraping fires exactly when Twitter's DOM is ready.
// ─────────────────────────────────────────────────────────────

let lastUrl         = "";
let pendingAnalysis = null;

// ── Helper: parse "12.4K" / "1.2M" → real number ─────────────
function parseCount(str) {
  if (!str) return 0;
  str = str.replace(/,/g, "").trim();
  if (/K$/i.test(str)) return Math.round(parseFloat(str) * 1_000);
  if (/M$/i.test(str)) return Math.round(parseFloat(str) * 1_000_000);
  if (/B$/i.test(str)) return Math.round(parseFloat(str) * 1_000_000_000);
  return parseInt(str) || 0;
}
...
// ── Core fix: wait for a DOM element instead of guessing time ─
// Resolves with the element when it appears, or null on timeout.
// Never rejects — missing elements degrade gracefully.
...
function scrapeProfileData() {
  ...
  const nameEl = document.querySelector(
    '[data-testid="UserName"] span span, [data-testid="UserName"] div span'
  );
  d.displayName = nameEl?.innerText?.trim() || null;
  ...
  // Location / website
  d.hasLocation = !!(document.querySelector('[data-testid="UserLocation"]')?.innerText?.trim());
  d.hasWebsite  = !!(document.querySelector('[data-testid="UserUrl"]')?.innerText?.trim());
  ...
  d.accountAgeDays = null;
  if (d.joinDateText) {
    const m = d.joinDateText.match(/Joined\s+(.+)/i);
    if (m) {
      const parsed = new Date(m[1]);
      if (!isNaN(parsed))
        d.accountAgeDays = Math.floor((Date.now() - parsed.getTime()) / 86_400_000);
    }
  }

  // Followers & following — fallback selector chain survives Twitter DOM changes
  const followersRaw = trySelectors([
    'a[href$="/verified_followers"] span span',
    'a[href$="/followers"] span span',
    'a[href$="/followers"] span',
  ]);
  const followingRaw = trySelectors([
    'a[href$="/following"] span span',
    'a[href$="/following"] span',
  ]);

  d.followersRaw = followersRaw || "0";
  d.followingRaw = followingRaw || "0";
  d.followers    = parseCount(d.followersRaw);
  d.following    = parseCount(d.followingRaw);
  d.ratio        = d.following > 0
    ? parseFloat((d.followers / d.following).toFixed(2))
  ...
  // Profile photo
  const photoSrc    = document.querySelector('[data-testid^="UserAvatar-Container"] img')?.src || "";
  d.hasDefaultPhoto = photoSrc === "" || photoSrc.includes("default_profile");
  ...
  // Verified badge
  d.isVerified = !!(document.querySelector('[data-testid="icon-verified"]'));

  // Pinned tweet
  const pinnedEl   = document.querySelector('[data-testid="socialContext"]');
  d.hasPinnedTweet = pinnedEl?.innerText?.toLowerCase().includes("pinned") || false;

  // Tweets — only scored when we have enough of them
  const tweetEls    = document.querySelectorAll('[data-testid="tweetText"]');
  d.visibleTweets   = Array.from(tweetEls).slice(0, 10).map(el => el.innerText.trim());
  d.tweetCount      = d.visibleTweets.length;
  const uniqueSet   = new Set(d.visibleTweets);
  // null = not enough data, don't penalise
  d.repetitionRatio = d.tweetCount >= 3
    ? parseFloat(((d.tweetCount - uniqueSet.size) / d.tweetCount).toFixed(2))
    : null;
  ...
  // Confidence flag — did key data actually load?
  d.dataConfidence = (d.followers > 0 || d.following > 0) && d.accountAgeDays !== null
    ? "high" : "low";
...
// ── Step 2: Score ─────────────────────────────────────────────
//
//  All weights are calibrated so a perfectly human profile
//  reaches ~100 and a fully-empty bot profile reaches ~0
//  WITHOUT relying on the clamp. The clamp is a safety net only.
//
//  Signal                  Max pts   Direction
//  ─────────────────────────────────────────────
//  1. Account age            22      + or −
//  2. Profile photo          18      + or −
//  3. Follower/follow ratio  16      + or −
//  4. Bio presence           14      + or −
//  5. Tweet repetition       12      − only
//  6. Verified badge         10      + only
//  7. Completeness extras     8      + or −
//  8. Absolute followers      6      − only
//  9. Pinned tweet            4      + only
//  ─────────────────────────────────────────────
...
function scoreProfile(d) {
  ...
  // ── 1. Account age (±22) ──────────────────────────────────
  if (d.accountAgeDays === null) {
    // No data — stay neutral
  } else if (d.accountAgeDays < 7) {
    score -= 22; flags.push("Account under 7 days old");
  } else if (d.accountAgeDays < 30) {
    score -= 16; flags.push("Account under 30 days old");
  } else if (d.accountAgeDays < 90) {
    score -= 8;  flags.push("Account under 3 months old");
  } else if (d.accountAgeDays >= 730) {
    score += 22; positives.push("Account over 2 years old");
  } else if (d.accountAgeDays >= 365) {
    score += 14; positives.push("Account over 1 year old");
  } else {
    score += 4;  // 90–365 days: young but established
  }

  // ── 2. Profile photo (±18) ────────────────────────────────
  if (d.hasDefaultPhoto) {
    score -= 18; flags.push("No profile photo");
  } else {
    score += 18; positives.push("Custom profile photo");
  }

  // ── 3. Follower / following ratio (±16) ───────────────────
  if (d.following > 500 && d.ratio < 0.05) {
    score -= 16; flags.push("Mass following, almost no followers");
  } else if (d.following > 1000 && d.ratio < 0.1) {
    score -= 10; flags.push("High following, very low follower ratio");
  } else if (d.following > 200 && d.ratio < 0.2) {
    score -= 4;
  } else if (d.ratio >= 2.0) {
    score += 16; positives.push("Strong follower ratio (2×+)");
  } else if (d.ratio >= 1.0) {
    score += 10; positives.push("Good follower ratio");
  } else if (d.ratio >= 0.4) {
    score += 5;
  }

  // ── 4. Bio presence and quality (±14) ─────────────────────
  if (!d.hasBio) {
    score -= 14; flags.push("No bio");
  } else if (d.bio && d.bio.length < 15) {
    score += 4;  flags.push("Bio is very short");
  } else {
    score += 14; positives.push("Bio present");
  }

  // ── 5. Tweet repetition (−12 max, no positive) ────────────
  // Only applied when we have at least 3 tweets scraped.
  // Repetitive tweets are a strong bot spam pattern.
  if (d.repetitionRatio !== null) {
    if (d.repetitionRatio > 0.5 && d.tweetCount >= 5) {
      score -= 12; flags.push("Highly repetitive tweets");
    } else if (d.repetitionRatio > 0.3 && d.tweetCount >= 4) {
      score -= 6;  flags.push("Some repetitive tweets");
    }
  }

  // ── 6. Verified badge (+10) ───────────────────────────────
  // Lowered from +20 — verification is purchasable, not proof of humanity.
  if (d.isVerified) {
    score += 10; positives.push("Verified account");
  }

  // ── 7. Profile completeness extras (±8) ───────────────────
  // Header, location, website are each small positive signals.
  // Combined absence on an already-suspicious profile amplifies the penalty.
  let extras = 0;
  if (d.hasHeaderImage) extras += 3;
  if (d.hasLocation)    extras += 2;
  if (d.hasWebsite)     extras += 3;
  score += extras;
  if (extras === 0 && !d.hasBio && d.hasDefaultPhoto) {
    score -= 4; flags.push("Completely empty profile");
  }

  // ── 8. Absolute follower count (−6 max, no positive) ──────
  // High follower counts can be purchased, so no bonus.
  // Very few followers on an old account is suspicious.
  if (d.accountAgeDays !== null && d.accountAgeDays > 180 && d.followers < 5) {
    score -= 6; flags.push("Almost no followers for account age");
  } else if (d.accountAgeDays !== null && d.accountAgeDays > 90 && d.followers < 2) {
    score -= 4;
  }

  // ── 9. Pinned tweet (+4) ──────────────────────────────────
  if (d.hasPinnedTweet) {
    score += 4; positives.push("Has pinned tweet");
  }

  // Safety clamp — should rarely hit with calibrated weights
  score = Math.max(5, Math.min(95, Math.round(score)));
...
function showOverlay(result, d) {
  ...
  overlay.style.cssText = `
    position:absolute;top:70px;right:16px;z-index:99999;
    width:272px;background:#fff;border:1.5px solid #e1e8ed;
    border-radius:16px;padding:16px;
    font-family:-apple-system,BlinkMacSystemFont,"Segoe UI",Roboto,sans-serif;
    font-size:13px;color:#0f1419;
    box-shadow:0 2px 12px rgba(0,0,0,0.10);
  ...
  overlay.innerHTML = `
    ...
    <div style="text-align:center;padding:10px 0 12px">
      <div style="font-size:32px;font-weight:700;color:${result.color};line-height:1.2">
        ${result.score}<span style="font-size:15px;font-weight:400">/100</span>
      </div>
      <div style="font-size:18px;font-weight:700;color:${result.color};margin-top:4px">
        ${result.label}
      </div>
    </div>

    <div style="font-size:11px;color:#8b98a5;text-align:center;margin-top:8px">
      HumanityCheck · ${new Date().toLocaleTimeString()}
    </div>
  `;
...
// ── Step 4: Smart wait → scrape → score → display ─────────────
//
//  THE KEY FIX:
//  Instead of setTimeout(fn, 4000) which is a blind guess,
//  we wait for the specific DOM element that Twitter injects
//  last — the followers link. When that exists, the full
//  profile API response has arrived and everything else is ready.
//
async function runProfileAnalysis() {
  console.log("[HumanityCheck] Waiting for profile DOM...");

  // 1. Wait for follower link — slowest element, signals full profile load
  await waitForElement(
    'a[href$="/followers"], a[href$="/verified_followers"]',
    8000
  );

  // 2. Extra 150ms tick — Twitter fills the follower count text in a
  //    second React render after the <a> tag appears. Without this,
  //    parseCount() sees an empty span and returns 0.
  await new Promise(r => setTimeout(r, 150));

  // 3. Also wait for tweets, but don't block forever — give it 3s max.
  //    If no tweets appear (e.g. protected account), we skip repetition scoring.
  await Promise.race([
    waitForElement('[data-testid="tweetText"]', 3000),
    new Promise(r => setTimeout(r, 3000)),
  ]);

  const data   = scrapeProfileData();
  const result = scoreProfile(data);
  console.log("[HumanityCheck] Score:", result.score, result.label, data);
  showOverlay(result, data);
}
...
function onNavigate() {
  ...
  // Cancel any in-flight analysis for the previous page
  if (pendingAnalysis) {
    pendingAnalysis.cancelled = true;
  ...
  // Tag this run so mid-navigation cancellation works
  const token = { cancelled: false };
  pendingAnalysis = token;

  runProfileAnalysis().then(() => {
    if (token.cancelled) {
      console.log("[HumanityCheck] Stale analysis discarded");
      document.getElementById("hc-overlay")?.remove();
    }
  });
}

// Watch for Twitter SPA navigation (pushState / replaceState changes
// don't fire popstate, so we use a MutationObserver on the body)
const navObserver = new MutationObserver(onNavigate);
navObserver.observe(document.body, { childList: true, subtree: true });
onNavigate(); // Run once on initial page load